...
需要布局边界或自定义性能标记时，可通过 `onDebugOverlay` 注入调试框。回调在业务 `compose` 完成后执行，不会改变业务状态；Release 构建默认不启用：

```cpp
static const DslAppConfig config = DslAppConfig{}
    .showDebugOverlay(true)
    .onDebugOverlay([](eui::Ui& ui, const eui::Screen& screen) {
        components::layoutDebugOverlay(ui, "debug.root", screen.width, screen.height, 8.0f, "root");
    });
```
